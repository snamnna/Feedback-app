 ...
 */
  ...
  test("find request and accept user", async ({ page }) => {
    ...
    let liElements = await page
      .locator('li:has(div[data-testid="course-card"])')
      .elementHandles(); // Look for course cards
    let liElementsLength = await page
      .locator('li:has(div[data-testid="course-card"])')
      .count(); // Count course cards

    if (liElementsLength > 0) {
      // Continue if courses were found
      courseFound = true;
      ...
      for (let i = 1; i <= liElementsLength; i++) {
        let liElement = page.locator(
          `li:has(div[data-testid="course-card"]):nth-child(${i})`
        ); // Locate the i-th course card
        ...
        try {
          await page.waitForSelector('li:has(button:has-text("Accept"))', {
            state: "attached",
            timeout: 5000,
          });
        } catch (error) {
          console.log(
            "No enrollment requests found. Returning to Dashboard and trying again.\n"
          );
        }
        ...
        if (!(await noEnrollments.isVisible())) {
          // Continue if "no enrollments" text wasn't found
          ...
          if (firstPendingRequest) {
            ...
            const acceptButton = await firstPendingRequest.$(
              'button:has-text("Accept")'
            );
            await acceptButton?.click();
            console.log(
              `Enrollment accepted for student with ID: ${studentId}`
            );
            break;
          }
        } else {
